...
def get_guess(mask_exclude):
    ...
    options = np.where(np.logical_not(mask_exclude[row_idx, idx_flat]))[0] + 1
...
while np.sum(s == 0) > 0:
    ...
    for guess in options:
        ...
        if is_valid(s):
            ...
            logging.debug(f"Assume guess {guess} at row {row_idx} col {col_idx} is valid")

        else:
            logging.debug(f"Assume guess {guess} at row {row_idx} col {col_idx} is invalid")
